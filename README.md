...
Для работы с учебным проектом достаточно зарегистрировать аккаунт и не подтверждать 
его — аккаунт будет находиться в тестовом режиме.
...
Настройка на Yandex Cloud. операционная система Ubuntu
Обновление системы
...
После открытия портов проверьте настройки файрвола, чтобы убедиться, что правила применились:
sudo ufw status
В результате выполнения этой команды вы должны увидеть, что порты 80 и 443 находятся в состоянии 
ALLOW
, что означает разрешение входящих соединений через эти порты.
